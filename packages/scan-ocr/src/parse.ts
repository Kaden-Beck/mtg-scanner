...
const DIGITS_ONLY = /^\d+$/;
const COLLECTOR_TOKEN = /^\d{1,4}[a-zA-Z★†]?$/;
const STAR_COLLECTOR = /^\d+[★†]$/;
const SLASH_PATTERN = /\b(\d{1,4}[a-zA-Z★†]?)\s*\/\s*\d{1,4}\b/;
const BARE_NUMBER = /\b(\d{1,4}[a-zA-Z★†]?)\b/;
...
export function isRejectedOcrText(raw: string): boolean {
  ...
  if (cleaned.length < 3 || cleaned.length > 30) return true;
...
export function parseCollectorNumber(raw: string): ParsedCollectorNumber {
  ...
  if (slash?.[1]) {
    const setFromEnd = tokens.find((t) => SET_LIKE.test(t) && !DIGITS_ONLY.test(t));
    return {
  ...
  // Pattern: "SET R 0125" or "SET 0125" (rarity letter optional)
  let setCode: string | null = null;
  let collectorNumber: string | null = null;

  for (const t of tokens) {
    if (SET_TOKEN.test(t) && !DIGITS_ONLY.test(t) && !RARITY_LETTERS.has(t.toUpperCase())) {
      // Prefer first set-like token.
      if (setCode === null && t.length >= 3) {
  ...
  // Bare number alone — set unknown.
  if (collectorNumber === null) {
    const bare = BARE_NUMBER.exec(cleaned);
    if (bare?.[1]) collectorNumber = bare[1];
  }

  // Skip rarity letter sitting between set and number: "FDN U 0125"
  if (setCode === null) {
    for (let i = 0; i < tokens.length - 1; i += 1) {
      const a = tokens[i];
      const b = tokens[i + 1];
      if (a && b && SET_LIKE.test(a) && RARITY_LETTERS.has(b.toUpperCase())) {
        setCode = a.toLowerCase();
        break;
      }
      if (a && SET_LIKE.test(a) && !RARITY_LETTERS.has(a.toUpperCase())) {
        setCode = a.toLowerCase();
        break;
      }
    }
